...
if [ $success -ne 0 ]
then
    echo "Backuping MongoDB failed"
    if [ $sendgridEnabled -ne 0 ]
    then
        echo "Sending email..."
        ./sendgrid.py "`uname -n`: MongoDB backup failed" "$logFile" "$sendgridLogin" "$sendgridPassword" "$sendgridFrom" "$sendgridTo"
        echo "Done"
    fi
    exit $success
fi
...
then
    ...
    ./sendgrid.py "`uname -n`: MongoDB backup weekly report" "$logFile" "$sendgridLogin" "$sendgridPassword" "$sendgridFrom" "$sendgridTo"
    echo "Done"
fi
